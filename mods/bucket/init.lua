...
local S = MultiCraft.get_translator("bucket")
...
local function check_protection(pos, name, text)
	if MultiCraft.is_protected(pos, name) then
		MultiCraft.log("action", (name ~= "" and name or "A mod")
			.. " tried to " .. text
			.. " at protected position "
			.. MultiCraft.pos_to_string(pos)
			.. " with a bucket")
		MultiCraft.record_protection_violation(pos, name)
		return true
	end
	return false
end
	...
	if itemname ~= nil then
		MultiCraft.register_craftitem(itemname, {
			...
			on_place = function(itemstack, user, pointed_thing)
				...
				local node = MultiCraft.get_node_or_nil(pointed_thing.under)
				local ndef = node and MultiCraft.registered_nodes[node.name]
				...
				if ndef and ndef.buildable_to then
					...
					node = MultiCraft.get_node_or_nil(lpos)
					local above_ndef = node and MultiCraft.registered_nodes[node.name]
				...
				MultiCraft.set_node(lpos, {name = source})
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				if inv:room_for_item("main", {name=liquiddef.itemname}) then
					...
					MultiCraft.add_item(pos, liquiddef.itemname)
				end
			...
			if liquiddef.force_renew then
				source_neighbor =
					MultiCraft.find_node_near(pointed_thing.under, 1, liquiddef.source)
			end

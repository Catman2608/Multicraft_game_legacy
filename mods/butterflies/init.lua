...
local S = MultiCraft.get_translator("butterflies")
...
for i in ipairs (butter_list) do
	...
	MultiCraft.register_node("butterflies:butterfly_"..name, {
		description = desc,
		drawtype = "plantlike",
		tiles = {{
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if not MultiCraft.is_protected(pos, player_name) and
				...
				MultiCraft.set_node(pos, {name = "butterflies:butterfly_"..name})
		...
		on_timer = function(pos, elapsed)
			if MultiCraft.get_node_light(pos) < 11 then
				MultiCraft.set_node(pos, {name = "butterflies:hidden_butterfly_"..name})
			end
			MultiCraft.get_node_timer(pos):start(30)
		end
	})

	MultiCraft.register_node("butterflies:hidden_butterfly_"..name, {
		drawtype = "airlike",
...
-- get decoration ID
local butterflies = MultiCraft.get_decoration_id("butterflies:butterfly")
MultiCraft.set_gen_notify({decoration = true}, {butterflies})

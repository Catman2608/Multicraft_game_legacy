...
function carts:is_rail(pos, railtype)
	...
	if node == "ignore" then
		...
		node = MultiCraft.get_name_from_content_id(data[vi])
	end
	if MultiCraft.get_item_group(node, "rail") == 0 then
		return false
	end
	if not railtype then
		return true
	end
	return MultiCraft.get_item_group(node, "connect_to_raillike") == railtype
end
...
function carts:get_rail_groups(additional_groups)
	...
	local groups = {
		...
		attached_node = 1,
		rail = 1,
		connect_to_raillike = MultiCraft.raillike_group("rail")
	}

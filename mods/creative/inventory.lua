...
function creative.init_creative_inventory(player)
	...
	MultiCraft.create_detached_inventory("creative_" .. player_name, {
		allow_move = function(inv, from_list, from_index, to_list, to_index, count, player2)
			local name = player2 and player2:get_player_name() or ""
			if not MultiCraft.is_creative_enabled(name) or
					to_list == "main" then
				return 0
			end
			return count
		...
		allow_take = function(inv, listname, index, stack, player2)
			local name = player2 and player2:get_player_name() or ""
			if not MultiCraft.is_creative_enabled(name) then
				return 0
			end
			return -1
		end,
		...
		on_take = function(inv, listname, index, stack, player2)
			if stack and stack:get_count() > 0 then
				MultiCraft.log("action", player_name .. " takes " .. stack:get_name().. " from creative inventory")
			end
		end,
	}, player_name)
...
local function description(def, lang_code)
	...
	if lang_code then
		s = MultiCraft.get_translated_string(lang_code, s)
	end
	return s:gsub("\n.*", "") -- First line only
end
...
function creative.update_creative_inventory(player_name, tab_content)
	local inv = player_inventory[player_name] or
			creative.init_creative_inventory(MultiCraft.get_player_by_name(player_name))
	local player_inv = MultiCraft.get_inventory({type = "detached", name = "creative_" .. player_name})
	...
	local lang
	local player_info = MultiCraft.get_player_information(player_name)
...
local trash = MultiCraft.create_detached_inventory("trash", {
...
function creative.register_tab(name, title, items)
	sfinv.register_page("creative:" .. name, {
		...
		is_in_nav = function(self, player, context)
			return MultiCraft.is_creative_enabled(player:get_player_name())
		end,
		get = function(self, player, context)
			...
			return sfinv.make_formspec(player, context,
				"label[5.8,4.15;" .. MultiCraft.colorize("#FFFF00", tostring(pagenum)) .. " / " .. tostring(pagemax) .. "]" ..
				[[
...
MultiCraft.register_on_mods_loaded(function()
	for name, def in pairs(MultiCraft.registered_items) do
		...
		if group.node or (nogroup and MultiCraft.registered_nodes[name]) then
			registered_nodes[name] = def
		elseif group.tool or (nogroup and MultiCraft.registered_tools[name]) then
			registered_tools[name] = def
		elseif group.craftitem or (nogroup and MultiCraft.registered_craftitems[name]) then
			registered_craftitems[name] = def
		end
	end
end)

creative.register_tab("all", S("All"), MultiCraft.registered_items)
...
function sfinv.get_homepage_name(player)
	if MultiCraft.is_creative_enabled(player:get_player_name()) then
		return "creative:all"
	else

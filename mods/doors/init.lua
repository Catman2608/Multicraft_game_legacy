...
local S = MultiCraft.get_translator("doors")
...
function doors.get(pos)
	...
	if doors.registered_doors[node_name] then
		...
		return {
			...
			state = function(self)
				local state = MultiCraft.get_meta(self.pos):get_int("state")
				return state %2 == 1
			end
		}
	elseif doors.registered_trapdoors[node_name] then
		...
		return {
			...
			state = function(self)
				return MultiCraft.get_node(self.pos).name:sub(-5) == "_open"
			end
		}
	else
		return nil
...
MultiCraft.register_node("doors:hidden", {
...
function doors.door_toggle(pos, node, clicker)
	local meta = MultiCraft.get_meta(pos)
	node = node or MultiCraft.get_node(pos)
	...
	if state % 2 == 0 then
		MultiCraft.sound_play(def.door.sounds[1],
			{pos = pos, gain = def.door.gains[1], max_hear_distance = 10}, true)
	else
		MultiCraft.sound_play(def.door.sounds[2],
...
function doors.register(name, def)
	...
	MultiCraft.register_lbm({
		...
		action = function(pos, node)
			...
			MultiCraft.swap_node(pos, {name = name .. "_" .. new.type, param2 = p2})
			...
			MultiCraft.set_node({x = pos.x, y = pos.y + 1, z = pos.z},
				{name = "doors:hidden", param2 = p3})
		end
	})

	MultiCraft.register_craftitem(":" .. name, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			local pdef = MultiCraft.registered_nodes[node.name]
			...
			if pdef and pdef.buildable_to then
				...
				pdef = MultiCraft.registered_nodes[node.name]
			...
			local top_node = MultiCraft.get_node_or_nil(above)
			local topdef = top_node and MultiCraft.registered_nodes[top_node.name]
			...
			if MultiCraft.is_protected(pos, pn) or MultiCraft.is_protected(above, pn) then
				return itemstack
			end

			local dir = placer and MultiCraft.dir_to_facedir(placer:get_look_dir()) or 0
			...
			local state = 0
			if MultiCraft.get_item_group(MultiCraft.get_node(aside).name, "door") == 1 then
				state = state + 2
				MultiCraft.set_node(pos, {name = doorname .. "_b", param2 = dir})
				MultiCraft.set_node(above, {name = "doors:hidden", param2 = (dir + 3) % 4})
			else
				MultiCraft.set_node(pos, {name = doorname .. "_a", param2 = dir})
				MultiCraft.set_node(above, {name = "doors:hidden", param2 = dir})
			end
			...
			if not MultiCraft.is_creative_enabled(pn) then
				itemstack:take_item()
			end

			MultiCraft.sound_play(def.sounds.place, {pos = pos}, true)

			on_place_node(pos, MultiCraft.get_node(pos),
	...
	def.after_dig_node = function(pos, node, meta, digger)
		MultiCraft.remove_node({x = pos.x, y = pos.y + 1, z = pos.z})
		MultiCraft.check_for_falling({x = pos.x, y = pos.y + 1, z = pos.z})
	end
	...
	if def.protected then
		...
		def.on_skeleton_key_use = function(pos, player, newsecret)
			...
			if owner ~= pname then
				MultiCraft.record_protection_violation(pos, pname)
				MultiCraft.chat_send_player(pname, S("You do not own this locked door."))
				return nil
			end
	...
	def.mesh = "door_a.obj"
	MultiCraft.register_node(":" .. name .. "_a", table.copy(def))

	def.mesh = "door_b.obj"
	MultiCraft.register_node(":" .. name .. "_b", table.copy(def))

	def.mesh = "door_a2.obj"
	MultiCraft.register_node(":" .. name .. "_c", table.copy(def))

	def.mesh = "door_b2.obj"
	MultiCraft.register_node(":" .. name .. "_d", table.copy(def))
...
function doors.register_door(name, def)
	...
	if not def.tiles then
		...
		MultiCraft.log("warning", modname .. " registered door \"" .. name .. "\" " ..
...
function doors.trapdoor_toggle(pos, node, clicker)
	...
	if string.sub(node.name, -5) == "_open" then
		MultiCraft.sound_play(def.sound_close,
			{pos = pos, gain = def.gain_close, max_hear_distance = 10}, true)
		MultiCraft.swap_node(pos, {name = string.sub(node.name, 1,
			string.len(node.name) - 5), param1 = node.param1, param2 = node.param2})
	else
		MultiCraft.sound_play(def.sound_open,
			{pos = pos, gain = def.gain_open, max_hear_distance = 10}, true)
		MultiCraft.swap_node(pos, {name = node.name .. "_open",
...
function doors.register_trapdoor(name, def)
	...
	if def.protected then
		...
		def.after_place_node = function(pos, placer, itemstack, pointed_thing)
			...
			return MultiCraft.is_creative_enabled(pn)
		end
		...
		def.on_skeleton_key_use = function(pos, player, newsecret)
			...
			if owner ~= pname then
				MultiCraft.record_protection_violation(pos, pname)
				MultiCraft.chat_send_player(pname, S("You do not own this trapdoor."))
				return nil
			end
	...
	MultiCraft.register_node(name_opened, def_opened)
	MultiCraft.register_node(name_closed, def_closed)
...
function doors.register_fencegate(name, def)
	local fence = {
		...
		on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			local node_def = MultiCraft.registered_nodes[node.name]
			MultiCraft.swap_node(pos, {name = node_def._gate, param2 = node.param2})
			MultiCraft.sound_play(node_def._gate_sound, {pos = pos, gain = 0.15,
	...
	MultiCraft.register_node(":" .. name .. "_closed", fence_closed)
	MultiCraft.register_node(":" .. name .. "_open", fence_open)

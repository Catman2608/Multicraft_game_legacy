...
MultiCraft.override_item("default:dirt", {
	soil = {
		base = "default:dirt",
		dry = "farming:soil",
...
MultiCraft.override_item("default:dirt_with_grass", {
	soil = {
...
MultiCraft.override_item("default:dirt_with_dry_grass", {
	soil = {
...
MultiCraft.override_item("default:dirt_with_rainforest_litter", {
	soil = {
...
MultiCraft.override_item("default:dirt_with_coniferous_litter", {
	soil = {
...
MultiCraft.override_item("default:dry_dirt", {
	soil = {
...
MultiCraft.override_item("default:dry_dirt_with_dry_grass", {
	soil = {
...
MultiCraft.register_node("farming:dry_soil", {
...
MultiCraft.register_node("farming:dry_soil_wet", {
...
MultiCraft.override_item("default:desert_sand", {
...
MultiCraft.register_node("farming:desert_sand_soil", {
...
MultiCraft.register_node("farming:desert_sand_soil_wet", {
...
MultiCraft.register_node("farming:straw", {
	description = S("Straw"),
...
MultiCraft.register_abm({
	...
	action = function(pos, node)
		local n_def = MultiCraft.registered_nodes[node.name] or nil
		...
		pos.y = pos.y + 1
		local nn = MultiCraft.get_node_or_nil(pos)
		if not nn or not nn.name then
			return
		end
		local nn_def = MultiCraft.registered_nodes[nn.name] or nil
		pos.y = pos.y - 1

		if nn_def and nn_def.walkable and MultiCraft.get_item_group(nn.name, "plant") == 0 then
			MultiCraft.set_node(pos, {name = base})
			return
		end
		-- check if there is water nearby
		local wet_lvl = MultiCraft.get_item_group(node.name, "wet")
		if MultiCraft.find_node_near(pos, 3, {"group:water"}) then
			-- if it is dry soil and not base node, turn it into wet soil
			if wet_lvl == 0 then
				MultiCraft.set_node(pos, {name = wet})
			end
		else
			-- only turn back if there are no unloaded blocks (and therefore
			-- possible water sources) nearby
			if not MultiCraft.find_node_near(pos, 3, {"ignore"}) then
				-- turn it back into base if it is already dry
				if wet_lvl == 0 then
					-- only turn it back if there is no plant/seed on top of it
					if MultiCraft.get_item_group(nn.name, "plant") == 0 and MultiCraft.get_item_group(nn.name, "seed") == 0 then
						MultiCraft.set_node(pos, {name = base})
					end

				-- if its wet turn it back into dry soil
				elseif wet_lvl == 1 then
					MultiCraft.set_node(pos, {name = dry})
				end
			end
		end
	end,
...
for i = 1, 5 do
	MultiCraft.override_item("default:grass_"..i, {drop = {
		max_items = 1,
		items = {
...
MultiCraft.override_item("default:junglegrass", {drop = {
	max_items = 1,
	items = {
...
MultiCraft.register_node("farming:cotton_wild", {

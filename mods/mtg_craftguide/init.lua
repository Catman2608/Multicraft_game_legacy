local S = MultiCraft.get_translator("mtg_craftguide")
...
local function groups_to_item(groups)
	if #groups == 1 then
		...
		elseif MultiCraft.registered_items["default:"..group] then
...
local function get_craftable_recipes(output)
	local recipes = MultiCraft.get_all_craft_recipes(output)
	if not recipes then
		return nil
	end
	...
	for i = #recipes, 1, -1 do
		for _, item in pairs(recipes[i].items) do
			...
			if not MultiCraft.registered_items[item] then
...
local function cache_usages(recipe)
	...
	for _, item in pairs(recipe.items) do
		if not added[item] then
			...
			elseif show_item(MultiCraft.registered_items[item]) then
...
local function is_fuel(item)
	return MultiCraft.get_craft_result({method="fuel", items={item}}).time > 0
end
...
local function item_button_fs(fs, x, y, item, element_name, groups)
	...
	if groups then
		...
		if not tooltip then
			local groupstr = {}
			for _, group in ipairs(groups) do
				table.insert(groupstr, MultiCraft.colorize("yellow", group))
			end
			groupstr = table.concat(groupstr, ", ")
			tooltip = S("Any item belonging to the group(s): @1", groupstr)
		end
	elseif is_fuel(item) then
		local itemdef = MultiCraft.registered_items[item:match("%S*")]
		local desc = itemdef and itemdef.description or S("Unknown Item")
		tooltip = desc.."\n"..MultiCraft.colorize("orange", S("Fuel"))
	end
	if tooltip then
...
local function recipe_fs(fs, data)
	...
	if shapeless or recipe.method == "cooking" then
		...
		local tooltip = shapeless and S("Shapeless") or
			S("Cooking time: @1", MultiCraft.colorize("yellow", cooktime))
...
local function get_formspec(player)
	...
	table.insert(fs,
		...
		"label[5.8,4.15;"..MultiCraft.colorize("yellow", data.pagenum).." / "..
			data.pagemax.."]"..
...
local function execute_search(data)
	...
	for _, item in ipairs(init_items) do
		local def = MultiCraft.registered_items[item]
		local desc = def and MultiCraft.get_translated_string(data.lang_code, def.description)

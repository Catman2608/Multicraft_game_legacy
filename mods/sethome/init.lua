...
local S = MultiCraft.get_translator("sethome")
...
local function loadhomes()
	...
	for pos, name in input:read("*a"):gmatch("(%S+ %S+ %S+)%s([%w_-]+)[\r\n]") do
		homepos[name] = MultiCraft.string_to_pos(pos)
	end
	input:close()
end
...
sethome.set = function(name, pos)
	...
	player_meta:set_string("sethome:home", MultiCraft.pos_to_string(pos))
...
sethome.get = function(name)
	local player = MultiCraft.get_player_by_name(name)
	local player_meta = player:get_meta()
	local pos = MultiCraft.string_to_pos(player_meta:get_string("sethome:home"))
	if pos then
		return pos
	end
...
MultiCraft.register_privilege("home", {

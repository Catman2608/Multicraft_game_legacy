...
if mg_name == "v6" or mg_name == "singlenode" or
		MultiCraft.settings:get("static_spawnpoint") or
		MultiCraft.settings:get_bool("engine_spawn") then
	return
end
...
local biome_ids = {
	MultiCraft.get_biome_id("taiga"),
	MultiCraft.get_biome_id("coniferous_forest"),
	MultiCraft.get_biome_id("deciduous_forest"),
	MultiCraft.get_biome_id("grassland"),
	MultiCraft.get_biome_id("savanna"),
}
...
local mapgen_limit = tonumber(MultiCraft.get_mapgen_setting("mapgen_limit"))
...
local function search()
	for iter = 1, checks do
		local biome_data = MultiCraft.get_biome_data(pos)
		...
		for id_ind = 1, #biome_ids do
			...
			if biome == biome_id then
				local spawn_y = MultiCraft.get_spawn_level(pos.x, pos.z)
				if spawn_y then
...
local enable_bed_respawn = MultiCraft.settings:get_bool("enable_bed_respawn")

...
local S = MultiCraft.get_translator("tnt")


-- Default to enabled when in singleplayer
local enable_tnt = MultiCraft.settings:get_bool("enable_tnt")
if enable_tnt == nil then
	enable_tnt = MultiCraft.is_singleplayer()
end
...
local tnt_radius = tonumber(MultiCraft.settings:get("tnt_radius") or 3)

-- Fill a list with data for content IDs, after all nodes are registered
local cid_data = {}
MultiCraft.register_on_mods_loaded(function()
	for name, def in pairs(MultiCraft.registered_nodes) do
		cid_data[MultiCraft.get_content_id(name)] = {
			name = name,
			drops = def.drops,
...
local function rand_pos(center, pos, radius)
	local def
	local reg_nodes = MultiCraft.registered_nodes
	local i = 0
	repeat
		...
		def = reg_nodes[MultiCraft.get_node(pos).name]
		i = i + 1
...
local function eject_drops(drops, pos, radius)
	...
	for _, item in pairs(drops) do
		...
		while count > 0 do
			...
			local obj = MultiCraft.add_item(drop_pos, dropitem)
			if obj then
...
local function destroy(drops, npos, cid, c_air, c_fire,
	...
	if not ignore_protection and MultiCraft.is_protected(npos, owner) then
		return cid
	end
	...
	elseif def.flammable then
		...
		local node_drops = MultiCraft.get_node_drops(def.name, "")
...
local function entity_physics(pos, radius, drops)
	local objs = MultiCraft.get_objects_inside_radius(pos, radius)
	for _, obj in pairs(objs) do
		...
		if obj:is_player() then
			...
			if luaobj then
				...
				local entity_drops = {}
				local objdef = MultiCraft.registered_entities[luaobj.name]
...
function tnt.burn(pos, nodename)
	local name = nodename or MultiCraft.get_node(pos).name
	local def = MultiCraft.registered_nodes[name]
	if not def then
		return
	elseif def.on_ignite then
		def.on_ignite(pos)
	elseif MultiCraft.get_item_group(name, "tnt") > 0 then
		MultiCraft.swap_node(pos, {name = name .. "_burning"})
		MultiCraft.sound_play("tnt_ignite", {pos = pos}, true)
...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	local count = 0
	local c_tnt
	local c_tnt_burning = MultiCraft.get_content_id("tnt:tnt_burning")
	local c_tnt_boom = MultiCraft.get_content_id("tnt:boom")
	local c_air = MultiCraft.get_content_id("air")
	if enable_tnt then
		c_tnt = MultiCraft.get_content_id("tnt:tnt")
	else
	...
	basic_flame_on_construct = MultiCraft.registered_nodes["fire:basic_flame"].on_construct

	local c_fire = MultiCraft.get_content_id("fire:basic_flame")
	...
	for x = -radius * 1.5, radius * 1.5 do
		...
		if r / radius < 1.4 then
			MultiCraft.check_single_for_falling(s)
		end
	end
	end
	end
	...
	MultiCraft.log("action", "TNT owned by " .. owner .. " detonated at " ..
		MultiCraft.pos_to_string(pos) .. " with radius " .. radius)
...
function tnt.boom(pos, def)
	...
	if not def.explode_center and def.ignore_protection ~= true then
		MultiCraft.set_node(pos, {name = "tnt:boom"})
	end
	local sound = def.sound or "tnt_explode"
	MultiCraft.sound_play(sound, {pos = pos, gain = 2.5,
	...
	MultiCraft.log("action", "A TNT explosion occurred at " .. MultiCraft.pos_to_string(pos) ..
		" with radius " .. radius)
end

MultiCraft.register_node("tnt:boom", {
	drawtype = "airlike",
...
MultiCraft.register_node("tnt:gunpowder", {
	...
	groups = {dig_immediate = 2, attached_node = 1, flammable = 5,
		connect_to_raillike = MultiCraft.raillike_group("gunpowder")},
	sounds = default.node_sound_leaves_defaults(),

	on_punch = function(pos, node, puncher)
		if puncher:get_wielded_item():get_name() == "default:torch" then
			MultiCraft.set_node(pos, {name = "tnt:gunpowder_burning"})
			MultiCraft.log("action", puncher:get_player_name() ..
...
MultiCraft.register_node("tnt:gunpowder_burning", {
	drawtype = "raillike",
	paramtype = "light",
	sunlight_propagates = true,
	walkable = false,
	...
	on_construct = function(pos)
		MultiCraft.sound_play("tnt_gunpowder_burning", {pos = pos,
			gain = 2}, true)
		MultiCraft.get_node_timer(pos):start(1)
	end,
})
...
MultiCraft.register_craftitem("tnt:tnt_stick", {
...
function tnt.register_tnt(def)
	...
	if enable_tnt then
		MultiCraft.register_node(":" .. name, {
			...
			on_punch = function(pos, node, puncher)
				if puncher:get_wielded_item():get_name() == "default:torch" then
					MultiCraft.swap_node(pos, {name = name .. "_burning"})
					MultiCraft.registered_nodes[name .. "_burning"].on_construct(pos)
					MultiCraft.log("action", puncher:get_player_name() ..
	...
	MultiCraft.register_node(":" .. name .. "_burning", {
		tiles = {
			{
				name = tnt_burning,
				animation = {
		...
		on_construct = function(pos)
			MultiCraft.sound_play("tnt_ignite", {pos = pos}, true)
			MultiCraft.get_node_timer(pos):start(4)

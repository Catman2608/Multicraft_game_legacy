...
local S = MultiCraft.get_translator("xpanes")


local function is_pane(pos)
	return MultiCraft.get_item_group(MultiCraft.get_node(pos).name, "pane") > 0
end

local function connects_dir(pos, name, dir)
	local aside = vector.add(pos, MultiCraft.facedir_to_dir(dir))
	if is_pane(aside) then
		return true
	end

	local connects_to = MultiCraft.registered_nodes[name].connects_to
	if not connects_to then
		return false
	end
	local list = MultiCraft.find_nodes_in_area(aside, aside, connects_to)
...
local function swap(pos, node, name, param2)
	...
	MultiCraft.swap_node(pos, {name = name, param2 = param2})
end
...
MultiCraft.register_on_placenode(function(pos, node)
	if MultiCraft.get_item_group(node, "pane") then
		update_pane(pos)
	end
	for i = 0, 3 do
		local dir = MultiCraft.facedir_to_dir(i)
		update_pane(vector.add(pos, dir))
	end
end)

MultiCraft.register_on_dignode(function(pos)
	for i = 0, 3 do
		local dir = MultiCraft.facedir_to_dir(i)
		update_pane(vector.add(pos, dir))
	end
...
function xpanes.register_pane(name, def)
	for i = 1, 15 do
		MultiCraft.register_alias("xpanes:" .. name .. "_" .. i, "xpanes:" .. name .. "_flat")
	end

	local flatgroups = table.copy(def.groups)
	flatgroups.pane = 1
	MultiCraft.register_node(":xpanes:" .. name .. "_flat", {
	...
	MultiCraft.register_node(":xpanes:" .. name, {
		drawtype = "nodebox",
		paramtype = "light",
		is_ground_content = false,
		sunlight_propagates = true,
...
if MultiCraft.get_modpath("doors") then
